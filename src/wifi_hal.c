...
#if defined (_COSA_QCA_ARM_)
#define IPQ_DEFAULT_MAX_CHAINS 15
#define SCAN_EBUSY_RETRY_COUNT   3
#define SCAN_EBUSY_RETRY_DELAY_US 200000  /* 200ms */
#endif
...
#elif defined(TCXB8_PORT) || defined(XB10_PORT) || defined(SCXER10_PORT) || defined(VNTXER5_PORT) || defined(SCXF10_PORT) || defined(_COSA_QCA_ARM_)
...
INT wifi_hal_setRadioOperatingParameters(wifi_radio_index_t index, wifi_radio_operationParam_t *operationParam)
{
    ...
    wifi_hal_info_print("%s:%d: setRadioOperatingParams primary iface=%s"
        " mld_name='%s' mld_enable=%d enable=%d\n",
        __func__, __LINE__,
        wifi_hal_get_interface_name(primary_interface),
        primary_interface->mld_name,
        wifi_hal_is_mld_enabled(primary_interface),
        operationParam->enable);
#if defined(_COSA_QCA_ARM_)
    /* MLO per-link interfaces (mld0/mld1/mld6) must never be brought UP here.
     * They must stay DOWN until NL80211_CMD_ADD_LINK is called in
     * nl80211_update_wiphy / update_hostap_mlo.  Only the shared MLD AP
     * interface (phy00-mld0) needs to be UP, and that is handled in
     * interface_info_handler.  Enabling the per-link interface here causes
     * the kernel to return -ENOTUNIQ on subsequent ADD_LINK calls. */
    if (!wifi_hal_is_mld_enabled(primary_interface)) {
        ...
        wifi_hal_info_print("%s:%d: skipping enable of MLO link iface=%s"
            " (mld_enable=1, per-link iface must stay DOWN until ADD_LINK)\n",
            __func__, __LINE__,
            wifi_hal_get_interface_name(primary_interface));
    }
#else
...
INT wifi_hal_createVAP(wifi_radio_index_t index, wifi_vap_info_map_t *map)
{
    ...
    for (i = 0; i < map->num_vaps; i++) {
        ...
        if ((vap->vap_mode == wifi_vap_mode_sta) || (mld_name == NULL)) {
#endif
        ...
        if (vap->vap_mode == wifi_vap_mode_ap) {
            ...
            hapd->conf->ppe_vp_type = intf_offload_type;
            if (hapd && hapd->conf && (hapd->conf->ppe_vp_type > 0)) {
                wifi_hal_dbg_print("%s:%d: Setting PPE VP type %u for vap %s\n",
                        __func__, __LINE__, hapd->conf->ppe_vp_type, interface->name);
                wifi_hal_set_intf_offload_type(hapd->drv_priv, OUI_QCA,
             ...
             * Set the PPE VP type via vendor command directly to avoid kernel panic */
            {
                ...
                wifi_hal_set_intf_offload_type(interface, OUI_QCA,
                        QCA_NL80211_VENDOR_SUBCMD_SET_WIFI_CONFIGURATION,
                        NULL, 0, 0, NULL, interface->name,
                        intf_offload_type, true);
            }
...
#if defined(_COSA_QCA_ARM_) && defined(CONFIG_MLO)
                /*
...
#if defined(_COSA_QCA_ARM_) && defined(QCA_UD_HOSTAPD)
    res = wifi_drv_send_mlme(interface, buf, 24 + data_len, 1, freq, 0, 0, NULL, 0, 0, wait, link_id);
#else
    /* Upstream 2.12: no rate/rate_type */
