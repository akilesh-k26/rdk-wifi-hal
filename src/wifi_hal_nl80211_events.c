...
#if !defined(_COSA_QCA_ARM_) /* can be removed after rebasing RDK hostapd patches */
...
static void nl80211_ch_switch_notify_event(wifi_interface_info_t *interface, struct nlattr **tb, wifi_chan_eventType_t wifi_chan_event_type)
{
...
#endif
#if defined(_COSA_QCA_ARM_) && defined(QCA_UD_HOSTAPD)
    int punct_bmap = 0, pwr_mode_6ghz = 0, bw_device = 0, cf_device = 0;
#endif
    ...
    if (tb[NL80211_ATTR_PUNCT_BITMAP]) {
        punct_bmap = nla_get_u32(tb[NL80211_ATTR_PUNCT_BITMAP]);
    }

    if (tb[NL80211_ATTR_6G_REG_POWER_MODE]) {
        pwr_mode_6ghz = nla_get_u8(tb[NL80211_ATTR_6G_REG_POWER_MODE]);
    }

    if (tb[NL80211_ATTR_CHANNEL_WIDTH_DEVICE]) {
        bw_device = nla_get_u32(tb[NL80211_ATTR_CHANNEL_WIDTH_DEVICE]);
    }

    if (tb[NL80211_ATTR_CENTER_FREQ_DEVICE]) {
        cf_device = nla_get_u32(tb[NL80211_ATTR_CENTER_FREQ_DEVICE]);
    }
#endif /* _COSA_QCA_ARM_ && QCA_UD_HOSTAPD */
